...
SARIMA = csvread('D:/Thesis/data/forecast/Seasonal_ARIMA/SARIMA.csv', 1, 1);
LSTM = load('D:/Thesis/data/forecast/LSTM/LSTM_15min.mat');
LSTM = LSTM.out;
...
sensor1 = reshape(LSTM(1,:,:),8640, want_predict_time);
sensor19 = reshape(LSTM(19,:,:),8640, want_predict_time);
u_SARIMA = SARIMA(start_time:end_time,:)';
u_LSTM = reshape(LSTM(:,:,end), 19, 8640);
u_LSTM = u_LSTM(:,start_time:end_time);

initial = [density09(length(density09)-(past_time+want_predict_time-1):end,:); density10(1:288,:)]';
left = initial(1,:);
right = initial(end,:);
