function [u_grid, u_pre] = Traffic_KF(history, left, past_time, want_predict_time, update_time, dt)
% MacNicholas model
...
R = 0.05*eye(J); % observation error covariance
nsteps = update_time; % do assimilation every nsteps steps
p = past_time;
w = want_predict_time;

left_inter = interpolation(left, N*(w+p)+1);
bottom = history(:,1);

% 邊界條件
u_grid = zeros(J+1, N*(w+p)+1);

for k = 1:w+p
    u = zeros(J+1, N+1);
    u(:,1) = interpolation(bottom(:,k), J+1);
    u(1,:) = left_inter(N*(k-1)+1:N*k+1);
    for n = 1:N
        A = zeros(J,J);
        b = zeros(J,1);
        for i = 3:J
