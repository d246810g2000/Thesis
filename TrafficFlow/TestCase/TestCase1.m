...
for n = 1:N-1
    for i = 2:J
        if x(i)/t(n+1)<Q(kl)
            u_exact(i,n+1) = kl;
        elseif x(i)/t(n+1)>Q(kr)
            u_exact(i,n+1) = kr;
        else 
            u_exact(i,n+1) = G(x(i)/t(n+1));
        end
    end
end
...
MAPPE_EX = atan(abs((u_exact(:,end)-u_EX)./u_exact(:,end)))*100;
MAPPE_KF = atan(abs((u_exact(:,end)-u_KF)./u_exact(:,end)))*100;
...
subplot(2,2,1.5);
set(gcf,'unit','normalized','position',[0,0.2,1,0.6])
plot(x(2:end-1), u_exact(2:end-1,end), 'Color', [0.9290 0.6940 0.1250], 'LineWidth', 2); hold on;
plot(x(2:end-1), u_EX(2:end-1), 'Color', [0.8500 0.3250 0.0980], 'LineWidth', 2);
plot(x(2:end-1), u_KF(2:end-1), 'Color', [0 0.4470 0.7410], 'LineWidth', 2);
legend('Ground truth', 'Explicit method', 'Kalman Filter', 'location', 'southwest');
ylabel('density (pcu/km)');
title('Prediction at 30 min')
set(gca, 'XTick', [-20:10:20], 'XTickMode','manual', 'FontSize', 24);
set(gca, 'YTick', [15, 20], 'YTickMode','manual', 'FontSize', 24);
axis([-20, 20, 15, 20.5]);

subplot(2,2,3);
set(gcf,'unit','normalized','position',[0,0.2,1,0.6])
plot(x(2:end-1), error_EX(2:end-1), 'Color', [0.8500 0.3250 0.0980], 'LineWidth', 2); hold on;
plot(x(2:end-1), error_KF(2:end-1), 'Color', [0 0.4470 0.7410], 'LineWidth', 2);
...
subplot(2,2,4);
set(gcf,'unit','normalized','position',[0,0.2,1,0.6])
plot(x(2:end-1), MAPPE_EX(2:end-1), 'Color', [0.8500 0.3250 0.0980], 'LineWidth', 2); hold on;
plot(x(2:end-1), MAPPE_KF(2:end-1), 'Color', [0 0.4470 0.7410], 'LineWidth', 2);
legend('Explicit method', 'Kalman Filter', 'location', 'northwest');
xlabel('x (km)');
ylabel('density (pcu/km)');
title('Prediction error at 30 min')
set(gca, 'XTick', [-20:10:20], 'XTickMode','manual', 'FontSize', 24);
set(gca, 'YTick', [0, 1, 2], 'YTickMode','manual', 'FontSize', 24);
axis([-20, 20, 0, 2]);